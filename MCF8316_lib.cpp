  ...
  ============================================================
   ...
   Hardware-Verdrahtung (Teensy 4.0)
     ...
     PWM-Out    →  Pin 33  →  MCF8316 SPEEDWAKE
...
constexpr uint32_t WAKE_PULSE_MS  = 30;        SPEEDWAKE kurz statisch HIGH zum Aufwecken
...
constexpr uint16_t REG_CT_FAULT_STATUS  = 0x0E2;   Controller Faults
 ...
 Konfiguriert die Anlaufphase (Align-Modus, Standard-Startup).
   Bits [2825]  ALIGN_SLOW_RAMP_RATE = 7  → 100 As Stromanstieg
   Bits [2421]  ALIGN_TIME           = 5  → 400 ms Ausrichtungszeit
   Bits [2017]  ALIGN_OR_SLOW_CURRENT_ILIMIT = 3 → 1,0 A Ausrichtstrom
 Alle anderen Bits = 0 (Default MTR_STARTUP = 00b = Align-Modus)

 Hinweis zum Default-Problem ALIGN_SLOW_RAMP_RATE = 0 (0,1 As) + ALIGN_TIME = 0 (10 ms)
 ergibt im Auslieferungszustand nur ca. 0,001 A effektiv → Motor richtet sich nicht aus!
 (0,1 As × 10 ms = 0,001 A – bei weitem nicht genug zur Ausrichtung)
constexpr uint32_t MOTOR_STARTUP1_CFG =
    (7UL  25)     ALIGN_SLOW_RAMP_RATE = 7 (100 As)
 ...
 Open-Loop-Beschleunigung und automatischer Wechsel zu Closed-Loop.
   Bits [3027]  OL_ILIMIT         = 5 → 2,0 A Stromgrenze im Open-Loop
   Bits [2623]  OL_ACC_A1         = 5 → 10 Hzs lineare Beschleunigung
   Bits [2219]  OL_ACC_A2         = 4 →  5 Hzs² quadratische Beschleunigung
   Bit  [18]     AUTO_HANDOFF_EN   = 1 → automatischer OL→CL Übergang (BEMF-basiert)
   Bits [20]    THETA_ERROR_RAMP_RATE = 5 → 0,5 °ms Winkelkorrektur-Rate
...
constexpr uint32_t MOTOR_STARTUP2_CFG =
    ...
    (5UL  23)     OL_ACC_A1             = 5 (10 Hzs)
    (4UL  19)     OL_ACC_A2             = 4 (5 Hzs²)
...
constexpr uint32_t CLOSED_LOOP1_CFG =
 ...
 Stromgrenzen erhöhen; Software-Strombegrenzer-Lock deaktivieren.
   Bits [3027]  ILIMIT           = 7 → 3,0 A Closed-Loop Strom-Sollwert-Grenze
   Bits [2623]  HW_LOCK_ILIMIT   = 7 → 3,0 A HW-Komparator-Schutzschwelle
   Bits [2219]  LOCK_ILIMIT      = 7 → 3,0 A ADC-Sperrstrom-Schwelle
   Bits [1815]  LOCK_ILIMIT_MODE = 9 → ILIMIT-Lock-Erkennung deaktiviert
...
constexpr uint32_t FAULT_CONFIG1_CFG =
 ...
 Hardware-Komparator-Lock-Erkennung deaktivieren.
   Bits [1815]  HW_LOCK_ILIMIT_MODE = 9 → HW-Lock deaktiviert
...
constexpr uint32_t FAULT_CONFIG2_CFG =
 ...
 ── PIN_CONFIG1 (0x0A4) ──────────────────────────────────────
 Bits [10]  SPEED_MODE = 01b  → PWM-Duty-Cycle auf SPEED-Pin steuert Motor
...
constexpr uint32_t EEPROM_WRITE_KEY     = 0x8A500000;
 ...
 ─────────────────────────────────────────────────────────────
  Control-Word Aufbau
  Format (24-Bit)
   [23]    OP_RW    0 = Schreiben, 1 = Lesen
   [22]    CRC_EN    0 = CRC deaktiviert
   [2120] DLEN      00=16bit, 01=32bit, 10=64bit
   [1916] MEM_SEC   0x0 für alle nutzbaren Register
   [1512] MEM_PAGE  0x0 für alle nutzbaren Register
   [110]  MEM_ADDR  Registeradresse (untere 12 Bit)
 ─────────────────────────────────────────────────────────────
static uint32_t buildControlWord(bool isRead, uint16_t addr) {
  uint32_t cw = 0;
  cw = (isRead  1UL  0UL)  23;    OP_RW
   Bit 22 CRC_EN = 0 (CRC deaktiviert)
  cw = 0b01UL  20;                   DLEN = 01b → 32-Bit Daten
   Bits 1916 MEM_SEC = 0x0
   Bits 1512 MEM_PAGE = 0x0
 ...
 ─────────────────────────────────────────────────────────────
  ...
  Sequenz (Datasheet 7.6.2.2)
    ...
    → CW[2316] → ACK → CW[158] → ACK → CW[70] → ACK
...
static bool i2cWrite32(uint16_t addr, uint32_t data) {
  ...
  for (uint8_t attempt = 0; attempt  I2C_RETRIES; attempt++) {
    Wire1.beginTransmission(MCF_I2C_ADDR);
     ...
     --- Control Word MSB zuerst (Byte [2316], [158], [70]) ---
    ...
    Serial.printf(  [WARN] I2C-Write NACK, Versuch %d%d, Addr=0x%03X, Err=%dn,
                  attempt + 1, I2C_RETRIES, addr, err);
    delay(RETRY_DELAY_MS);
  }

  Serial.printf([ERR]  I2C-Write endgültig fehlgeschlagen! Addr=0x%03Xn, addr);
  return false;
}
...
static bool i2cRead32(uint16_t addr, uint32_t &result) {
   Control Word mit OP_RW = 1 (Bit 23 gesetzt)
  uint32_t cw = buildControlWord(true, addr);

   Phase 1 Control Word senden (Bus-Richtung Write, RW-Bit = 0 im Adress-Byte!)
   Das OP_RW=1 ist im Control-Word, NICHT im I2C-Adress-Byte.
  for (uint8_t attempt = 0; attempt  I2C_RETRIES; attempt++) {
    Wire1.beginTransmission(MCF_I2C_ADDR);
    Wire1.write((uint8_t)((cw  16) & 0xFF));
    Wire1.write((uint8_t)((cw   8) & 0xFF));
    ...
    if (attempt == I2C_RETRIES - 1) {
      Serial.printf([ERR]  I2C-Read (CW-Phase) fehlgeschlagen! Addr=0x%03X, Err=%dn, addr, err);
      return false;
    }
    delay(RETRY_DELAY_MS);
  }
  ...
  if (received  4) {
    Serial.printf([ERR]  I2C-Read Nur %d4 Bytes empfangen! Addr=0x%03Xn, received, addr);
    return false;
  }
...
static bool mcf8316Init() {
  Serial.println([MCF8316] Starte I2C-Initialisierung ...);
   ...
   Datasheet DVDD muss stabil sein, BuckLDO müssen hochgefahren sein.
  delay(150);

   ── Schritt 1 MOTOR_STARTUP1 – Anlaufparameter ────────────
   Behebt das Default-Problem 0,1 As Rampe + 10 ms Align-Zeit
   ergibt im Auslieferungszustand praktisch 0 A → Motor richtet sich nicht aus.
   Neu 100 As Rampe, 400 ms Zeit, 1,0 A Strom → zuverlässige Ausrichtung.
  Serial.print([MCF8316] MOTOR_STARTUP1 (0x084) Align 100As, 400ms, 1A ... );
  if (!i2cWrite32(REG_MOTOR_STARTUP1, MOTOR_STARTUP1_CFG)) {
    Serial.println(FEHLER!);
    return false;
  }
  Serial.printf(OK (0x%08lX)n, (unsigned long)MOTOR_STARTUP1_CFG);
  delay(10);

   ── Schritt 2 MOTOR_STARTUP2 – Open-Loop-Parameter ────────
   Konfiguriert den Open-Loop-Betrieb 2 A Stromgrenze,
   10 Hzs Beschleunigung, automatischer Übergang zu Closed-Loop
   sobald BEMF erkannt wird (AUTO_HANDOFF_EN=1).
  Serial.print([MCF8316] MOTOR_STARTUP2 (0x086) OL 2A, 10Hzs, Auto-Handoff ... );
  if (!i2cWrite32(REG_MOTOR_STARTUP2, MOTOR_STARTUP2_CFG)) {
    Serial.println(FEHLER!);
    return false;
  }
  Serial.printf(OK (0x%08lX)n, (unsigned long)MOTOR_STARTUP2_CFG);
  delay(10);
  ...
  Serial.printf(OK (0x%08lX)n, (unsigned long)CLOSED_LOOP1_CFG);
  delay(10);
  ...
  Serial.printf(OK (0x%08lX)n, (unsigned long)FAULT_CONFIG1_CFG);
  delay(10);
  ...
  Serial.printf(OK (0x%08lX)n, (unsigned long)FAULT_CONFIG2_CFG);
  delay(10);
  ...
  Serial.printf(OK (0x%08lX)n, (unsigned long)PIN_CONFIG1_PWM_MODE);
  delay(10);
  ...
  if (i2cRead32(REG_GD_FAULT_STATUS, gdFault)) {
    Serial.printf(0x%08lX  %sn, (unsigned long)gdFault,
  ...
  if (i2cRead32(REG_CT_FAULT_STATUS, ctFault)) {
    Serial.printf(0x%08lX  %sn, (unsigned long)ctFault,
...
static void motorSetSpeed(uint8_t speed_pct) {
  ...
  const bool shouldSendWakePulse = (speed_pct  0 && !s_motorRunning);
   ...
   SPEEDWAKE-Pin braucht nach Standby einen stabilen HIGH-Pegel.
  ...
  if (shouldSendWakePulse) {
    Serial.printf([Motor] Geschwindigkeit %3d %%  (PWM = %3lu  255, Wake=%lums)n,
                  speed_pct, pwmVal, (unsigned long)WAKE_PULSE_MS);
  } else {
    Serial.printf([Motor] Geschwindigkeit %3d %%  (PWM = %3lu  255)n, speed_pct, pwmVal);
  }
...
void setup() {
  ...
  Serial.printf([PWM]   Pin %d, Frequenz = %lu Hz, Auflösung = 8-Bitn,
  ...
  Serial.printf([I2C]   Wire1 gestartet, Takt = %lu Hzn, (unsigned long)I2C_CLOCK_HZ);
  Serial.printf([I2C]   TARGET_ID = 0x%02Xnn, MCF_I2C_ADDR);

   ── MCF8316 initialisieren ─────────────────────────────────
  if (!mcf8316Init()) {
    Serial.println(n[HALT]  Initialisierung fehlgeschlagen!);
    Serial.println([HINT]  Prüfe I2C-Pull-ups (4,7 kΩ nach 3,3 V));
    Serial.println([HINT]  Prüfe MCF8316 Versorgungsspannung (VVM));
    Serial.println([HINT]  Prüfe Verdrahtung SDASCL (Wire1 Pin 1716));
    while (true) { delay(500); }
  }

  Serial.println(n[OK]    Initialisierung abgeschlossen.);
  Serial.println([INFO]  Strommodus aktiv CURRENT_REF = Duty% × 3A);
  Serial.println([INFO]  Testschleife 10 s Motor EIN (50 % → 1,5 A)  →  10 s Motor AUSn);
}
...
void loop() {
  ...
  for (uint32_t i = ON_DURATION_S; i  0; i--) {
    Serial.printf(  Motor läuft  ...  noch %2lu Sekundenn, i);
    delay(1000);
  }
  ...
  for (uint32_t i = OFF_DURATION_S; i  0; i--) {
    Serial.printf(  Motor gestoppt  ...  noch %2lu Sekundenn, i);
    delay(1000);
  }
}
